...
int gr_alloc_ipal(void)
{
	...
	if ((grd_ipal = (uchar *) gr_malloc(32768))==NULL) return RGB_OUT_OF_MEMORY;		// was gr_malloc
...
int gr_init_ipal(void)
{
  ...
  if ((data = (uchar *) gr_malloc(3*256)) == NULL) return RGB_OUT_OF_MEMORY;	// was gr_malloc
  ...
  for(i=0;i<256;++i) {
	...
	if ((dist_buf = (ulong *) gr_malloc(sizeof(ulong) * 32768))==NULL) return RGB_OUT_OF_MEMORY;	// was gr_malloc

	inv_cmap_2(256,colormap,5,dist_buf,grd_ipal);

 	gr_free(dist_buf);	// was gr_free
 	gr_free(data);	// was gr_free
