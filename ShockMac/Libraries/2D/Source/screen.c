...
grs_screen *gr_alloc_screen (short w, short h)
 {
  ...
  if ((p=(uchar *)gr_malloc(sizeof (*s)+2*sizeof (*c))) == NULL)	// was gr_malloc
     goto bailout2;
  if ((b = nvalloc (w, h)) == (uchar *)-1)      
  	goto bailout1;
...
void gr_free_screen (grs_screen *s)
{
	vfree (s->bm.bits);
	if (s->c->ytab)
        gr_free(s->c->ytab);	// was gr_free
  if ((s->c+1)->ytab)
      gr_free((s->c+1)->ytab);	// was gr_free
  gr_free(s->c);	// was gr_free
  gr_free(s);	// was gr_free
}
