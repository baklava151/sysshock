...
*/
    ...
    */
	...
	if (mouseInstantX != x || mouseInstantY != y)		// If different
	{
		mouseInstantX = x;												// save the position
		mouseInstantY = y;
		
		e.x = x;																// and inform the callback routines
		e.y = y;
		e.type = MOUSE_MOTION;
...
errtype mouse_init(short m, short n)
{
	mouse_state mstate;
/*
	union REGS regs;
...
//	eventMask = mDownMask;
	...
	if (mouseMask & (MOUSE_LUP | MOUSE_RUP | MOUSE_CUP))
		eventMask |= mUpMask;
    */
    int ifEvent = SDL_PollEvent(&mEvent);

    if (ifEvent)
    {
        res->x = mEvent.button.x;								// fill in the mouse_event record.
        res->y = mEvent.button.y;
        res->timestamp = mEvent.button.timestamp;

        if (mEvent.type == SDL_MOUSEBUTTONDOWN && (mEvent.button.button & SDL_BUTTON_RIGHT))
        {
            res->type = MOUSE_RDOWN;
            res->buttons = 2;
        }
        else if (mEvent.type == SDL_MOUSEBUTTONUP && (mEvent.button.button & SDL_BUTTON_RIGHT))
        {
            res->type = MOUSE_RUP;
            res->buttons = 2;
        }

        if (mEvent.type == SDL_MOUSEBUTTONDOWN && (mEvent.button.button & SDL_BUTTON_LEFT))
        {
            res->type = MOUSE_LDOWN;
            res->buttons = 1;
        }
        else if (mEvent.type == SDL_MOUSEBUTTONUP && (mEvent.button.button & SDL_BUTTON_LEFT))
        {
            res->type = MOUSE_LUP;
            res->buttons = 1;
        }
...
//	EventRecord	theEvent;
    ...
    int nowDown = SDL_GetMouseState(NULL, NULL);
    
	if (!gRBtnWasDown)											// If the right button was not already down
	{
		if (nowDown & SDL_BUTTON(SDL_BUTTON_RIGHT))													// and it is down now
		{
			rbType = MOUSE_RDOWN;							// trigger a MOUSE_RDOWN event.
			gRBtnWasDown = true;
		}
	}
	else if (!(nowDown & SDL_BUTTON(SDL_BUTTON_RIGHT)))												// If the right button was down but now is up
	{
	...
	if (rbType)														// Send a right button event
	{
		...
		res->x = x;												// Fill out the event record.
		res->y = y;
		res->type = rbType;
        // CREMEMBER
		//res->timestamp = theEvent.when;
...
*/
    ...
    int ifEvent = SDL_PollEvent(&event);

    if (ifEvent)
    {
        res->x = event.button.x;								// fill in the mouse_event record.
        res->y = event.button.y;
        res->timestamp = event.button.timestamp;
...
*/
    Uint32 buttons = SDL_GetMouseState(&pMouseState->x, &pMouseState->y);
