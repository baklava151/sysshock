...
AsmWideAdd:
    ...
    mov edx, [ecx+4]            ; low byte
    add edx, [eax+4]            ; add low bytes
    mov [eax+4], edx            ; store in target low byte

    mov edx, [eax]              ; high byte
    adc edx, [ecx]                ; add high bytes
    mov [eax], edx              ; store in target high byte
...
global AsmWideMultiply
AsmWideMultiply:
    mov eax, [esp+4]
    imul DWORD [esp+8]                    ;edx:eax
    mov ecx, eax
    mov eax, [esp+12]
    mov [eax+4], ecx
    mov [eax], edx
